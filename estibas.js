...
thumbnails.forEach(thumbnail => {
    thumbnail.addEventListener('click', event=>{
        ...
        imageContainer.style.backgroundImage = `url('./images/image-estiba-${event.target.id}.jpg')`
    });
});
...
modalthumbnails.forEach(modalthumbnail => {
    modalthumbnail.addEventListener('click', event=>{
        ...
        modalImageContainer.style.backgroundImage = `url('./images/image-estiba-${event.target.id.slice(-1)}.jpg')`
    });
});
...
function changeNextImage(imgcontainer){
    ...
    imageContainer.style.backgroundImage = `url('./images/image-estiba-${imgIndex}.jpg')`
}
